...
/**
 * Tests for token cracking resistance.
 * <p>
 ...
 * properties ensure that tokens remain secure even if large portions of the token
 ...
 *   <li>Multiple token generation - ensuring signatures are not predictable</li>
 * </ul>
 * <p>
...
class TokenCrackingResistanceTest {
    ...
    void tokensShouldBeResistantToBruteForceAttacks() {
        ...
        // Try to verify the token with a wrong signature
        ...
        Optional<?> result = tokenFactory.createAccessToken(tamperedToken);
        assertFalse(result.isPresent(), "Tampered token should be rejected");
    }
