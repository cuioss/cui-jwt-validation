...
*Implementation:* A comprehensive benchmark visualization system has been implemented and documented in link:benchmark-visualization.adoc[Benchmark Visualization Documentation]. The system includes interactive charts, detailed results tables, and performance badges. It defines key metrics (Score, Error, Unit), provides visualization templates, and references performance goals for baseline expectations.
...
*Implementation:* A GitHub Actions workflow has been created in `.github/workflows/benchmark.yml` that runs benchmarks on pushes to the main branch, version tag pushes, and can be manually triggered. The workflow stores benchmark results as artifacts, generates visualizations, and creates performance badges.
...
*Implementation:* A comprehensive benchmark comparison test has been implemented in `TokenValidatorComparisonTest.java`. This benchmark extends KeycloakITBase to set up a Keycloak test container and obtain real tokens. It then compares the validation performance of in-memory generated tokens versus real Keycloak tokens for access tokens, ID tokens, and refresh tokens. The benchmark includes both direct return methods and Blackhole consumption methods for more accurate measurement. The implementation includes detailed documentation about the purpose of the comparison and notes that it requires a running Keycloak instance via testcontainers, making it slower than standard benchmarks but valuable for validation purposes.
...
*Implementation:* A performance badge has been added to the README.adoc file showing the access token validation time. The badge is automatically updated by the benchmark workflow.
