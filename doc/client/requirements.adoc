...
`private_key_jwt` (RFC 7523) over a shared `client_secret`. `private_key_jwt` is the
implemented key-based method; mTLS (`tls_client_auth`, RFC 8705) is an alpha capability —
declared, never selected — and DPoP is the sender-constraining and client-authentication
direction. When a
