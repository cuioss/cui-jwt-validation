...
[#CLIENT-11]
=== CLIENT-11: Requesting sender-constrained tokens (DPoP)

The engine MUST be able to *request* sender-constrained tokens at the token endpoint by
presenting a DPoP proof (RFC 9449), so retrieved tokens cannot be replayed by a bearer that
stole them. DPoP is the implemented direction and the only token-request route this
requirement mandates: it binds the issued token to the client's proof key without
authenticating the client, so it composes with `private_key_jwt` (<<CLIENT-4>>) rather than
replacing it.

Requesting an mTLS (RFC 8705) certificate-bound token is *not* a capability this engine has,
and this requirement does not demand it: obtaining that binding means presenting a client
certificate at the token endpoint, and mTLS client authentication (`tls_client_auth`) is
alpha — declared, never selected, with no `SSLContext` client key material plumbed into the
transport (<<CLIENT-4>>). The certificate-bound case therefore stays specified only as a
*binding kind the server-side lifecycle preserves* where a token already carries one
(<<CLIENT-18>>) — preserving a `cnf.x5t#S256` binding is a capability distinct from the alpha
client-authentication method.

The *validation* of the resulting binding is inherited from the validation module
(`VALIDATION-8.7`); this requirement is the client-side delta of *asking for* the constraint
and preserving it (<<CLIENT-18>>).
...
| Shared secrets are the weakest link; strong auth is available as `private_key_jwt`. mTLS
  ...
  DPoP (RFC 9449) is the sender-constraining direction and is explicitly not a
  client-authentication method.
