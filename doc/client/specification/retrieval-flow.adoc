...
the *strongest configured method the AS also advertises* — never a method the caller did not
configure (xref:../requirements.adoc#CLIENT-4[CLIENT-4],
...
| `private_key_jwt` | Preferred — the key-based method ranked first among the configured strategies; asymmetric client assertion, no shared secret in transit. | https://www.rfc-editor.org/rfc/rfc7523[RFC 7523]
| mTLS (`tls_client_auth`) | Alpha — declared so an advertised entry resolves, never selected; the transport plumbs no client key material, leaving `private_key_jwt` as the supported strong client-authentication route. DPoP (RFC 9449) is the sender-constraining direction, not a client-authentication method (<<_sender_constrained_tokens>>). | https://www.rfc-editor.org/rfc/rfc8705[RFC 8705]
...
* *DPoP* (RFC 9449) — the implemented direction and the only token-request route specified
  here: the engine presents a DPoP proof on the token request so the issued access token is
  bound to its proof key (`cnf.jkt`).
* *mTLS* (RFC 8705) — requesting a certificate-bound token is *not* a capability this engine
  has: obtaining that binding means presenting a client certificate at the token endpoint,
  and mTLS client authentication (`tls_client_auth`) is alpha
  (<<_client_authentication>>). The certificate-bound case stays specified only as a
  *binding kind the server-side lifecycle preserves* where a token already carries one
  (xref:../requirements.adoc#CLIENT-18[CLIENT-18]) — preserving a `cnf.x5t#S256` binding is a
  capability distinct from the alpha client-authentication method.
