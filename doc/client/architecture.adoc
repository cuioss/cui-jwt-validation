...
declared, never selected), PAR, DPoP sender-constraining — requesting a certificate-bound token
is not a capability the engine has, while preserving a certificate binding a token already
carries is, and that preservation is a capability distinct from mTLS client authentication and
unaffected by its alpha status — stronger `nonce`, in-module hardened
