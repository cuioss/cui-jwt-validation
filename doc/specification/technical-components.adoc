...
`ParsedRefreshToken` implements `Serializable` and provides basic functionality for handling refresh tokens, which are treated as opaque strings according to OAuth2 specifications.
