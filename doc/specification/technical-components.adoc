...
* `Optional<KeyInfo> getKeyInfo(String kid)` - Gets a key by its ID
* `Optional<KeyInfo> getFirstKeyInfo()` - Gets the first available key
* `List<KeyInfo> getAllKeyInfos()` - Gets all available keys with their algorithms
