...
* link:../../src/main/java/de/cuioss/jwt/validation/TokenValidatorConfig.java[TokenValidatorConfig]
...
These values are also used as defaults in the `TokenValidatorConfig` class.
...
* `TokenValidatorConfig` is immutable after construction (using Lombok's `@Value` annotation)
...
2. Custom token size limits are correctly passed to `NonValidatingJwtParser` through `TokenValidatorConfig`
