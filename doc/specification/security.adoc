...
* Expiration Time (exp)
* Issued At (iat)
* Audience (aud) - validated if expected audience is provided
...
* The `ClaimValidator` class validates the `azp` (authorized party) claim, which identifies the client the token was issued for.
* The `JwksAwareTokenParserImpl` class supports both audience (`aud`) and `azp` validation through the `ClaimValidator`.
* Validation of both claims is configurable through the `JwksAwareTokenParserImpl` builder:
