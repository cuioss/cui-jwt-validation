...
=== C1. Reduce Cognitive Complexity in TokenClaimValidator
[x] *Priority:* Medium

*Description:* Refactor the `validateAudience` method in `TokenClaimValidator` to reduce its cognitive complexity as flagged by Sonar.

*Rationale:* High cognitive complexity makes code harder to understand, maintain, and test. Breaking down complex methods into smaller, focused methods improves code quality and readability.
