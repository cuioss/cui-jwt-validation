...
import de.cuioss.jwt.validation.wellKnown.WellKnownDiscoveryException;
import java.net.URL; // For getIssuer example

// ...

try {
    String wellKnownUrl = "https://your-idp.com/realms/my-realm/.well-known/openid-configuration";
    WellKnownHandler wellKnownHandler = WellKnownHandler.fromWellKnownUrl(wellKnownUrl);

    // The handler now contains the discovered metadata, e.g.:
    // URL jwksUri = wellKnownHandler.getJwksUri().orElse(null);
    // URL issuer = wellKnownHandler.getIssuer().orElse(null);
...
----
The `WellKnownHandler.fromWellKnownUrl()` method will fetch the document, parse it, and validate key aspects like the issuer consistency. It may throw a `WellKnownDiscoveryException` if the process fails.
...
import java.net.URL; // Required for .map(URL::toString)

// Assuming jwksConfig is configured using wellKnownHandler as shown above
// And wellKnownHandler is available

String expectedIssuerFromDiscovery = wellKnownHandler.getIssuer()
    .map(URL::toString)
    .orElseThrow(() -> new IllegalStateException("Issuer not found in WellKnownHandler, cannot configure TokenValidator."));
...
=== Considerations
* **Initial Latency**: The call to `WellKnownHandler.fromWellKnownUrl()` involves a network request to fetch the discovery document. This adds a one-time latency during initialization.
