...
----
The `WellKnownHandler.builder().wellKnownUrl().build()` pattern will fetch the document, parse it, and validate key aspects like the issuer consistency. It may throw a `WellKnownDiscoveryException` if the process fails.
...
=== Considerations
* **Initial Latency**: The call to `WellKnownHandler.builder().wellKnownUrl().build()` involves a network request to fetch the discovery document. This adds a one-time latency during initialization.
