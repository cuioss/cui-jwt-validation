...
|JWTValidation-144 |WELLKNOWN |Accessibility check for %s URL '%s' returned HTTP status %s. It might be inaccessible. |Logged when an accessibility check for a URL returns a non-successful HTTP status code
|JWTValidation-145 |WELLKNOWN |Accessibility check for %s URL '%s' failed with IOException: %s. It might be inaccessible. |Logged when an accessibility check for a URL fails with an IOException
|JWTValidation-146 |WELLKNOWN |Accessibility check for %s URL '%s' was interrupted: %s. It might be inaccessible. |Logged when an accessibility check for a URL is interrupted
|JWTValidation-147 |WELLKNOWN |Accessibility check for %s URL '%s' failed with exception: %s. It might be inaccessible. |Logged when an accessibility check for a URL fails with an exception
|JWTValidation-148 |JWKS |Creating HttpJwksLoaderConfig with invalid JWKS URI. The loader will return empty results. |Logged when an invalid JWKS URI is provided to HttpJwksLoaderConfig
|JWTValidation-149 |JWKS |Invalid JWKS URL string provided: %s. The loader will be created but will return empty results. |Logged when an invalid JWKS URL string is provided to HttpJwksLoaderConfig
|===
...
|JWTValidation-204 |WELLKNOWN |Issuer validation failed. Document issuer '%s' (normalized to base URL for .well-known: %s://%s%s%s) does not match the .well-known URL '%s'. Expected path for .well-known: '%s'. SchemeMatch=%s, HostMatch=%s, PortMatch=%s (IssuerPort=%s, WellKnownPort=%s), PathMatch=%s (WellKnownPath='%s') |Logged when the issuer validation fails because the issuer in the discovery document does not match the well-known URL
|===
